                ...
                <table class="table align-items-center mb-0">
                  ...
                  <tbody v-if="this.users.length != 0">
                    <tr v-for="(user, index) in this.users" :key="index">
                        <td class="align-middle text-center text-sm"><p class="text-xs font-weight-bold mb-0">{{ user.id }}</p></td>
                        <td ><p class="text-xs font-weight-bold mb-0">{{ user.name }}</p></td>
                        <td><p class="text-xs font-weight-bold mb-0">{{ user.email }}</p></td>
                        <td class="align-middle text-center text-sm"><p class="text-xs font-weight-bold mb-0">{{ user.phone }}</p></td>
                        <td class="align-middle text-center text-sm"><p class="text-xs font-weight-bold mb-0">{{ user.created_at }}</p></td>
                        
                        <td class="align-middle text-center">
                            <RouterLink :to="{path: '/users/'+user.id+'/edit'}"
                            class="btn btn-warning">
                            <span class="btn-inner--icon"><i class="material-icons">edit</i></span>
                                Edit
                            </RouterLink>
                            &nbsp;
                            <button class="btn btn-icon btn-3 btn-danger" @click="deleteStudent(user.id)" type="button">
...
export default{
    ...
    methods: {
        ...
        deleteStudent(userId){
            ...
            axios.delete(`http://127.0.0.1:8000/api/users/${userId}/delete`)
            .then(res =>{
